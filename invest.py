...
def print_header():
    ...
    print(Fore.CYAN + Style.BRIGHT + "     Conseiller d'investissement DCA avancé")
    print(Fore.CYAN + Style.BRIGHT + "=" * 50)
...
def get_user_choice():
    while True:
        ...
        print(Fore.WHITE + "1. Créer un nouveau profil d'investissement")
        print("2. Modifier un profil existant")
        print("3. Simuler la performance d'un portefeuille")
        print("4. Rééquilibrer un portefeuille existant")
        print("5. Comparer différentes stratégies d'investissement")
        print("6. Analyser l'historique des performances")
        print("7. Afficher le guide d'aide général")
        print("8. Quitter le programme")
        choice = input(Fore.CYAN + "Votre choix (1-8) : " + Fore.WHITE)
        if choice in ['1', '2', '3', '4', '5', '6', '7', '8']:
            return int(choice)
        else:
            print(Fore.RED + "Choix invalide. Veuillez entrer un nombre entre 1 et 8.")
...
def get_risk_profile():
    while True:
        ...
        print(Fore.WHITE + "1. Conservateur (faible risque, rendements potentiels plus bas)")
        print("2. Modéré (risque moyen, rendements potentiels moyens)")
        print("3. Agressif (risque élevé, rendements potentiels plus élevés)")
        choice = input(Fore.CYAN + "Votre choix (1-3) : " + Fore.WHITE)
        if choice in ['1', '2', '3']:
            return int(choice)
        else:
            print(Fore.RED + "Choix invalide. Veuillez entrer un nombre entre 1 et 3.")
            input(Fore.CYAN + "Appuyez sur Entrée pour continuer...")
    ...
    while True:
        try:
            ...
            if amount > 0:
                ...
                print(Fore.RED + "Le montant doit être supérieur à 0.")
        except ValueError:
...
def suggest_portfolio(risk, amount):
    ...
    for asset, allocation in portfolio.items():
        print(f"{Fore.CYAN}{asset}: {Fore.GREEN}{allocation}% ({amount * allocation / 100:.2f}€)")
    ...
    while True:
        try:
            ...
            if amount >= 0:
                ...
                print(Fore.RED + "Le montant doit être positif ou nul.")
        except ValueError:
            print(Fore.RED + "Veuillez entrer un nombre valide.")
...
def simulate_performance(portfolio, years, initial_investment, monthly_contribution):
    ...
    print(Fore.WHITE + f"Investissement initial : {initial_investment}€")
    print(Fore.WHITE + f"Contribution mensuelle : {monthly_contribution}€")
    ...
    for year in range(1, years + 1):
        ...
        print(f"{Fore.WHITE}Année {year}:")
        print(f"  {Fore.GREEN}Valeur du portefeuille: {total_value:.2f}€")
        print(f"  {Fore.YELLOW}Croissance: {year_growth*100:.2f}%")
        print(f"  {Fore.CYAN}Total investi: {total_invested:.2f}€")
        print(f"  {Fore.MAGENTA}Gain/Perte: {total_value - total_invested:.2f}€")
    ...
    print(f"{Fore.CYAN}L'historique des performances a été sauvegardé ici : {csv_path}")
...
def rebalance_portfolio(portfolio):
    ...
    print(Fore.WHITE + "\nPortefeuille actuel :")
    for asset, allocation in portfolio.items():
        print(f"{Fore.CYAN}{asset}: {Fore.GREEN}{allocation}%")
    ...
    for asset, target_allocation in portfolio.items():
        ...
        if adjustment > 0:
            print(f"{Fore.GREEN}Achetez {abs(adjustment * total_value / 100):.2f}€ de {asset}")
        elif adjustment < 0:
            print(f"{Fore.RED}Vendez {abs(adjustment * total_value / 100):.2f}€ de {asset}")
        else:
            print(f"{Fore.BLUE}Aucun ajustement nécessaire pour {asset}")
...
def modify_existing_profile(portfolio):
        ...
        if asset in portfolio:
            try:
                ...
                if 0 <= new_allocation <= 100:
                    ...
                    print(Fore.GREEN + "Allocation mise à jour.")
                else:
                    print(Fore.RED + "L'allocation doit être entre 0 et 100%.")
            except ValueError:
                print(Fore.RED + "Veuillez entrer un nombre valide.")
        else:
            print(Fore.RED + "Cet actif n'existe pas dans votre portefeuille.")
    ...
    for asset, allocation in portfolio.items():
        print(f"{Fore.CYAN}{asset}: {Fore.GREEN}{allocation:.2f}%")
...
def compare_strategies():
    ...
    years = int(input(Fore.YELLOW + "Sur combien d'années voulez-vous comparer les stratégies ? "))
...
def analyze_performance_history():
    ...
    if not os.path.exists(csv_path):
        print(Fore.RED + "Aucun historique de performance n'a été trouvé. Veuillez d'abord simuler la performance d'un portefeuille.")
        input(Fore.CYAN + "\nAppuyez sur Entrée pour revenir au menu principal...")
        return
    ...
    print(f"Croissance annuelle moyenne : {annual_growth:.2f}%")

    print(Fore.WHITE + "\nAnalyse des gains/pertes :")
    total_gain_loss = df["Gain/Perte"].iloc[-1]
    print(f"Gain/Perte total : {total_gain_loss:.2f}€")
    ...
    while True:
        ...
        if choice == 1:
            ...
            if portfolio:
                ...
                print(Fore.RED + "Veuillez d'abord créer un profil d'investissement (option 1).")
                input(Fore.CYAN + "Appuyez sur Entrée pour continuer...")
        elif choice == 3:
            if portfolio:
                years = int(input(Fore.YELLOW + "Sur combien d'années voulez-vous simuler la performance ? "))
                initial_investment = get_initial_investment()
                monthly_contribution = get_monthly_contribution()
                ...
                print(f"{Fore.YELLOW}Volatilité estimée du portefeuille : {volatility:.2f}%")
                print(Fore.WHITE + "La volatilité mesure la variation des rendements. Une volatilité plus élevée indique un risque plus important.")
